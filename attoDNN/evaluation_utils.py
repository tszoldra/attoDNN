            ...
            if train_test_split is not None and dataset.basename in train_dataset_name:
                ...
                if data_gen_fun is None or not use_data_generator:
                    ...
                    if predict_on_training_data:
                        ...
                        y_train, y_train_pred = get_generated_data_prediction(X_train, y_train, data_gen, num_data_gen_realizations)
                        grid[train_dataset_name][model_name][dataset.basename + '_train_aug'][model_number] = grid_record(y_train, y_train_pred)

                    y_test, y_test_pred = get_generated_data_prediction(X_test, y_test, data_gen, num_data_gen_realizations)

                    grid[train_dataset_name][model_name][dataset.basename + '_aug'][model_number] = grid_record(y_test, y_test_pred)
            else:
                if data_gen_fun is None or not use_data_generator:
                    ...
                    grid[train_dataset_name][model_name][dataset.basename + '_aug'][model_number] = grid_record(y_true, y_pred)
...
def ensembleNLL_predict(ensemble_dict, single_model=None):
    ...
    if single_model is None:
        ...
        if n > 0:
            y_pred = np.mean(y_pred_all, axis=0)
            y_pred_std = np.sqrt(np.mean(sigmasq_pred_all, axis=0) + np.mean(np.array(y_pred_all)**2, axis=0) - np.array(y_pred)**2)
    else:
        y_pred = ensemble_dict[single_model]['y_pred']
...
def regressionNLL_losses(y_true, y_pred, sigmasq_pred):
    ...
    nll = lambda y_true, y_pred, sigmasq_pred: 0.5 * (np.log(sigmasq_pred[:, 0]) + (y_true[:,0] - y_pred[:, 0])**2 / (sigmasq_pred[:, 0])) # samplewise loss
...
def regressionNLL_losses_with_uncertainty(y_true, y_pred, y_true_std, sigmasq_pred, n_samples=100):
    ...
    nlls = 0.5 * np.mean((np.log(sigmasq_pred[:,:]) + (y_true[:,:] - y_pred[:,:])**2 / (sigmasq_pred[:,:])), axis=1) # samplewise loss
                    ...
                    for k, eval_dataset_name in enumerate(eval_dataset_names):
                        if eval_dataset_name in grid[train_dataset_name][model_name].keys():
                            ...
                            if 'Bayesian' in model_name:
                                y_true, y_pred, y_pred_std, n = ensembleNLL_predict(result_model, single_model=plot_single_model_number)
                            else:
                                y_true, y_pred, y_pred_std, n = ensemble_predict(result_model, single_model=plot_single_model_number)
